...
- **Deploy policy:** GitHub-only (commit + push branch, open PR). Production deploy to `driverlog.link` (including `/info/*`) happens via Vercel automatically once a human reviews and merges the PR — the loop never pushes to `main` and never deploys directly.
