...
function rowToFleet(r) {
  return { id: r.id, name: r.name, ownerUserId: r.owner_user_id, createdAt: r.created_at };
}
...
export async function inviteMember(ownerUid, fleetId, email) {
  const sql = db();
  const fleetRows = await sql`select id from fleets where id = ${fleetId} and owner_user_id = ${ownerUid}`;
